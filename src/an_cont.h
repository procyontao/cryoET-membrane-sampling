...
inline void deleteContours( std::vector<IcontPtr> &conts );
inline void eraseContour( std::vector<IcontPtr> &conts, int idx );
...
int cont_generateTextAsConts( Iobj *obj, std::string text, Ipoint pos, float fontSize, int textAlign=TA_LEFT, bool smallCaps=false );
int cont_generateTextAreaAsConts( Iobj *obj, std::string text, Ipoint pos, float fontSize, int alignHoriz=TA_LEFT, int alignVert=TV_LEADING, bool smallCaps=false, float lineSpacing=4 );
...
int  cont_breakIntoSimple( std::vector<IcontPtr> &conts, Icont *cont );
...
int  cont_breakContourByContour( std::vector<IcontPtr> &contSegs, Icont *contO, Icont *contBreakO, float minDist );    // NEW
void cont_joinContsAtClosestApproach( Icont *newCont, Icont *cont1, Icont *cont2, bool addPtInMiddle );
void cont_joinContsAtClosestApproachVector( Icont *newCont, std::vector<IcontPtr> conts, bool addPtInMiddle );
void cont_getIntersectingConvexPolygon( Icont *newCont, Icont *cont1, Icont *cont2 );

int cont_breakContByZValue( Icont *contOrig, std::vector<IcontPtr> &contSegs, int zValue, bool removeOffSegments );  // NEW
int cont_breakOpenContAtZValue( Icont *contOrig, std::vector<IcontPtr> &contSegs, int zValueToBreak );               // NEW
int cont_breakContByCircle( Icont *contOrig, std::vector<IcontPtr> &contSegs, Ipoint *center, float radius );        // NEW
...
int cont_getIntersectingSegments( Icont *cont1, Icont *cont2, std::vector<IcontPtr> &cont1Segs, std::vector<IcontPtr> &cont2Segs  );   // MODIFY
int cont_getIntersectingSegmentsSafe( Icont *cont1, Icont *cont2, std::vector<IcontPtr> &cont1Segs, std::vector<IcontPtr> &cont2Segs  );   // MODIFY
int cont_getIntersectingPolygons( std::vector<IcontPtr> &finalConts, Icont *cont1, Icont *cont2 );
int cont_getUnionPolygons( std::vector<IcontPtr> &finalConts, Icont *cont1, Icont *cont2 );
...
std::vector<float> cont_getFractPtsAlongLength( Icont *cont, bool closed, int startPt );                                                    // NEW
int cont_addPtsFractsAlongLength( Icont *cont, Icont *contNew, std::vector<float> fractsAlongLen, bool closed, bool keepExistingPts, int startPt );      // NEW
