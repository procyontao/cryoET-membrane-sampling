...
#include <cmath>       // for some simple maths algorithms
...
const long long MAX_LONG      = 2147483647ll;     // largest negative value of a long long
const long long MIN_LONG      = 2147483648ll;     // largest positive value of a long long

const int MAX_INT      = 32767;      // largest negative value of a (singed) int
const int MIN_INT      = 32768;      // largest positive value of a (signed) int

const double DOUBL_MAX  =  1.7976931348623158e+308;     // largest pos double
const double DOUBL_MIN  = -1.7976931348623158e+308;     // largest neg double

const float FLOAT_MAX      =  3.40282e38f;       // largest positive value float
const float FLOAT_MIN_POS  =  1.17549e-38f;      // smallest positive value float
const float FLOAT_MIN      = -3.4027e35f;        // largest negative value float
...
template <typename type>  std::string toString( type value );
template <typename data>  std::string toStringPadNumber( data value, int padLength, char padChar='0' );
template <typename data>  std::string toStringWithCommas( data value );

inline std::string toString( float value, int decimal );
inline float string_getFloatFromString( std::string str );

inline std::string string_substr (std::string str, int chars, int offset );
inline std::string string_substrFromEnd (std::string str, int charsAtEnd);
inline bool string_startsWith (const std::string &haystack, const std::string &needle);

inline std::string string_replace ( std::string str, const std::string & searchStr, const std::string &replaceStr);
inline std::string string_eliminateDuplicates( const std::string &haystack, const std::string &needle);
inline std::vector<std::string> string_explode(std::string str, std::string explodeStr);
inline std::string string_explodeGetArgument( std::string str, std::string sepStr, int argNum, bool eliminateDuplicates=true);
...
inline std::string time_getCurrTimeStampString();
inline std::string time_getCurrTimeStampStringWithUnderscores();
inline long time_getCurrTimeInSecs();
inline std::string time_formatTimeElapsed( long secsElapsed, bool includeSecs, bool simpleFormat );


//## VECTOR FUNCTIONS:

template <typename type>  bool vector2D_transpose( std::vector< std::vector<type> > &v );
template <typename type>  void vector_eliminateDuplicates( std::vector<type> &v );
template <typename type>  bool vector_doesElementExistInVector( std::vector<type> v, type element );
template <typename type>  std::vector<type> vector_concat( std::vector<type> &v1, std::vector<type> &v2 );
template <typename type>  std::vector<type> vector_sort( std::vector<type> v, int startIdx, int endIdx=INT_MAX );
template <typename type>  std::vector<type> vector_sort( std::vector<type> v );
template <typename type>  std::vector<type> vector_reverse( std::vector<type> v, int startIdx=0, int endIdx=INT_MAX );


//## FILE FUNCTIONS:

inline bool file_saveStringToFile( std::string filePath, std::string text, bool append );
inline std::vector<std::string> file_loadTextFromFile( std::string filePath, bool printStatus=true );
...
inline std::string toStringPadNumber( data value, int padLength, char padChar )
{
	std::string returnStr = toString<const data &>(value);
...
template <typename data>
inline std::string toStringWithCommas( data value )
{
	...
	std::string returnStr = string_substr( str, leadingDigits, 0 );
...
//-------------
//-- Gets a substring (safely), from the end of the std::string.
...
inline std::string string_explodeGetArgument( std::string str, std::string sepStr, int argNum,
																							bool eliminateDuplicates )
{
	if(eliminateDuplicates)
		str = string_eliminateDuplicates( str, sepStr );

	std::vector<std::string> strVec = string_explode( str, sepStr );
...
inline std::string time_formatTimeElapsed( long secsElapsed, bool includeSecs, bool simpleFormat )
{
	...
	std::string formatted = ( (hours) ? toString(hours) + " hrs " : "" ) + toString(minutes) + " mins " + ( (includeSecs) ? toString(seconds) + " secs" : "");
...
template <typename type>
bool vector2D_transpose( std::vector< std::vector<type> > &v )
{
...
template <typename type>
void vector_eliminateDuplicates( std::vector<type> &v )
{
...
template <typename type>
inline bool vector_doesElementExistInVector( std::vector<type> v, type element )
{
...
template <typename type>
std::vector<type> vector_concat( std::vector<type> &v1, std::vector<type> &v2 )
{
	std::vector<type> returnVec = v1;
...
template <typename type>
inline std::vector<type> vector_sort( std::vector<type> v, int startIdx, int endIdx )
{
	if( startIdx < 0 )
		startIdx = 0;
...
template <typename type>
std::vector<type> vector_sort( std::vector<type> v )
{
	std::vector<type> returnVec = v;
...
template <typename type>
inline std::vector<type> vector_reverse( std::vector<type> v, int startIdx, int endIdx )
{
	if( startIdx < 0 )
		startIdx = 0;
	if( endIdx > (int)v.size()-1 )
...
inline std::vector<std::string> file_loadTextFromFile( std::string filePath, bool printStatus )
{
	...
	{
		if(printStatus)
			std::cout << "ERROR: Opening of file '" << filePath << "' failed." << std::endl;
		return text;
	}
...
inline bool file_saveStringToFile( std::string filePath, std::string text, bool append=false )
{
